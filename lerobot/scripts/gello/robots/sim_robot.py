...
class MujocoRobotServer:
    def __init__(
        ...
        mujoco_model_path = Path(xml_path)

        # 使用 from_xml_path 直接加载模型
        self._model = mujoco.MjModel.from_xml_path(mujoco_model_path.as_posix())
        self._data = mujoco.MjData(self._model)
        # self._data = mujoco.MjData(self._model)

        self._num_joints = self._model.nu

        self._joint_state = np.array([-0.12183455210997171 ,-0.002845966063342931, -0.06250428305273764 ,-2.0498354593763044 ,0.05259129256457092 ,1.9984585428810417 ,-0.9046528345737315, 1000 ])
