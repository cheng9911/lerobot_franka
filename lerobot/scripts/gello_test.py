...
def run_record(args,policy: torch.nn.Module | None = None,hydra_cfg: DictConfig | None = None,):
...
# Esc键（keyboard.Key.esc）：设置stop_recording和exit_early为True，停止数据记录并退出循环。
    ...
    while timestamp < args.warmup_time_s:
        ...
        if policy is None:
            ...
            observation = capture_observation(env,cameras)
            # raise NotImplementedError
            ...
            while timestamp < args.episode_time_s:
                ...
                if policy is not None:
                    with (
                        ...
                        for name in observation:
                            if "image" in name:
                                # 图像：uint8 HWC -> float32 [1, 3, H, W]
                                observation[name] = observation[name].type(torch.float32) / 255
                                observation[name] = observation[name].permute(2, 0, 1).contiguous()
                                if observation[name].dim() == 3:
                                    observation[name] = observation[name].unsqueeze(0)
                            else:
                                # 非图像：比如 state 是 [8] -> [1, 8]
                                if observation[name].dim() == 1:
                                    observation[name] = observation[name].unsqueeze(0)
                                elif observation[name].dim() == 3 and observation[name].shape[0] == 1:
                                    # 修复像 [1, 1, 8] 的情况 -> [1, 8]
                                    observation[name] = observation[name].squeeze(0)
...
def busy_wait(seconds):
    ...
    while time.perf_counter() < end_time:
        pass
def capture_observation(env, cameras):
...
if __name__ == "__main__":
    ...
    if args.mode == "record":
        ...
        if pretrained_policy_name_or_path is not None:
            ...
            run_record( args,policy, policy_cfg)
        else:
